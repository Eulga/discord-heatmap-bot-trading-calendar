...
@pytest.mark.asyncio
async def test_scheduler_skips_on_holiday(monkeypatch, capsys):
    state = {
        "commands": {"kheatmap": {"daily_posts_by_guild": {}, "last_images": {}}},
        "guilds": {"1": {"auto_screenshot_enabled": True}},
    }
    ...
    assert called["run"] == 0
    assert "reason=holiday" in out
    assert state["guilds"]["1"]["last_auto_skips"]["kheatmap"]["date"] == "2026-02-13"


@pytest.mark.asyncio
async def test_scheduler_skips_on_calendar_check_failure(monkeypatch, capsys):
    state = {
    ...
    out = capsys.readouterr().out
    assert "calendar-check-failed: calendar unavailable" in out
